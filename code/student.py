...
def build_vocabulary(image_paths, vocab_size):
    ...
    for path in image_paths:
        img = imread(path)
        descriptors = hog(img,  pixels_per_cell=(8,8),  cells_per_block=(4,4), feature_vector=True)
        descriptors = descriptors.tolist()
        bag_of_features.append(descriptors)
    bag_of_features = np.array(bag_of_features)
    bag_of_features = bag_of_features.reshape(-1, 4*4*9)
    vocab = MiniBatchKMeans(vocab_size).fit_predict(bag_of_features)
    return vocab
...
def get_bags_of_words(image_paths):
    ...
    for path in image_paths:
        img = imread(path)
        descriptors = hog(img,  pixels_per_cell=(8,8),  cells_per_block=(4,4), feature_vector=True)
        descriptors = descriptors.reshape(-1, 4*4*9)
        distance = cdist(vocab, descriptors, metric='euclidean')
        index = np.argmin(distance, axis=0)
        hist, bin_edges = np.histogram(idx, bins=len(vocab))
        norm = [float(i)/sum(hist) for i in hist]
        image_hists.append(norm)
    image_hists = np.asarry(image_hists)
    return image_hists
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    # TODO: Implement this function!
    SVC = LinearSVC(C=700.0, max_iter= 2000, random_state=0)
